...
def next_find_r(new_y,new_x,deq,count):
    global cnt
...
def next_find_c(new_y,new_x,deq,count):
    global cnt
...
def next_find_diag(new_y,new_x,deq,count):
    global cnt
    if 0<=new_y<N and 0<=new_x+1<N and not matrix[new_y][new_x+1]:
        if (new_y,new_x+1)==(N-1,N-1):
            cnt += 1
...
def bfs(y,x,shape,count):
    ...
    while deq:
        here = deq.popleft()
        y,x,shape,count = here[0],here[1],here[2],here[3],here[4]
        if shape == 0:
            next_find_r(y,x,deq,count)
        elif shape == 1:
            next_find_c(y,x,deq,count)
        else:
            next_find_diag(y,x,deq,count)
