    ...
    while schrenk: #고슴도치가 나중에 이동하므로 물이 다 찼아도 마지막에 고슴도치가 이동할 기회는 줘야함
        while cnt_w:
            ...
            for dir in range(4):
                ...
                if 0 <= new_y < r and 0 <= new_x < c and matrix[new_y][new_x] != 'X' and matrix[new_y][new_x] != 'D' and not visited_w[new_y][new_x]:
                    water.append((new_y, new_x))
                    visited_w[new_y][new_x] = 1
                    count_w += 1
        while cnt_s:
